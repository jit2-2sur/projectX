...
global seat_list
global selected_food_id
global selected_food_name
...
seat_list=[]
selected_food_id=0
selected_food_name='No movie selected'
...
def ticket(t_id):
    ...
    if ticket_data!=None:
        ...
        pdfbutton=Button(dialog_frame,text='print',bd=0,bg=sc_color,activebackground=sc_color,cursor='hand2',font=("Arial", 12),command=gen_pdf(t_id))
        pdfbutton.grid(row=9,column=2,sticky='w',padx=25,pady=(10,10))
...
class CounterButton(Button):
    def __init__(self, master=None, **kwargs):
        ...
        self.counter = 0
        self.config(text="0", command=self.increment_counter)
...
def order_food():
    ...
    i=0
    heading =Label(dialog_frame,text='select your favourite foods',fg=pr_color,bg=sc_color,font=('Microsoft YaHei UI Light',18,'bold'),bd=0)
    heading.grid(row=i,column=0,padx=10,pady=10)

    i,j=4,0
    Label(dialog_frame,text='enter amount of food',font=('Microsoft YaHei UI Light',10,'bold'),bg=sc_color,fg=pr_color).grid(row=3,column=1,sticky='w',padx=25,pady=(10,0))

    for food in foods:
        Label(dialog_frame,text=food,font=('Microsoft YaHei UI Light',10,'bold'),bg=sc_color,fg=pr_color).grid(row=i,column=0,sticky='w',padx=25,pady=(10,0))
        ordered_food[j]={'food':food,'price':price[j]}
        button = CounterButton(dialog_frame, text="Button " + str(j+1))
        button.grid(row=i,column=1,sticky='w',padx=25,pady=(10,0))
        buttons.append(button)
        i+=1
        j+=1

    signupButton=Button(dialog_frame,text='Pay now',font=('Microsoft YaHei UI Light',16,'bold'),bd=0,bg=pr_color,fg=sc_color,activebackground=pr_color,activeforeground=sc_color,width=17,cursor='hand2',command=get_all_counters)
    signupButton.grid(row=10,column=0,pady=10)
        
def get_all_counters():
    j=0
    for button in buttons:
        ordered_food[j]['qty']=button.get_counter_value()
        j+=1
    print(ordered_food)
    pay_food()
...
def pay_food():
    ...
    for i in range(len(ordered_food)):
        global food_bill
        food_bill+=(ordered_food[i]['price']*ordered_food[i]['qty'])
    print(food_bill)
    pay3()
...
def gen_ticket2():
    ...
    mycursor.execute(query)
    forder_list=mycursor.fetchall()
    fo_list=[]
    for i in forder_list:
        if i[0] != None:
            fo_list.append(i[0])
    print(fo_list)
    if len(fo_list)==0:
        ...
        new_food_order_id=max(fo_list)+1

    query='insert into food_order values(%s,%s,%s);'
    mycursor.execute(query,(new_food_order_id,food_bill,selected_user_id))
    disconnectDB()
    print('updation of status and user done')
    ####################################
    messagebox.showinfo('Successful','Payment done successfully')
    ...
    Label(dialog_frame,text='Your oder', font=("Arial", 12),bg='white',height=2).grid(row=0,column=1,sticky='w',padx=25,pady=(10,10))
    Label(dialog_frame,text='food_order_id', font=("Arial", 12),bg='white',height=2).grid(row=1,column=0,sticky='w',padx=25,pady=(10,10))
    Label(dialog_frame,text='total price', font=("Arial", 12),bg='white',height=2).grid(row=1,column=1,sticky='w',padx=25,pady=(10,10))
    Label(dialog_frame,text='ordered by : user_id', font=("Arial", 12),bg='white',height=2).grid(row=1,column=2,sticky='w',padx=25,pady=(10,10))
    for i,food in enumerate(forder_list):
        for j,val in enumerate(food):
            Label(dialog_frame,text=val, font=("Arial", 12),bg='white',height=2).grid(row=i+3,column=j,sticky='w',padx=25,pady=(10,10))
...
def pay3():
    ...
    Label(dialog_frame,text="choose one option", font=("Arial", 12),bg='white',height=2).place(x=240,y=100)
    Button(dialog_frame,text="online banking", font=("Arial", 12),bg='green',width=25,command=gen_ticket2).place(x=200,y=150)
    Button(dialog_frame,text="credit/debit card", font=("Arial", 12),bg='red',width=25,command=gen_ticket2).place(x=200,y=200)
    Button(dialog_frame,text="UPI", font=("Arial", 12),bg='purple',width=25,command=gen_ticket2).place(x=200,y=250)
    Button(dialog_frame,text="close", font=("Arial", 12),bg='white',width=25,command=destroy_dialog_frame).place(x=200,y=300)
...
def show_food():
    ...
    query='select * from food'
    mycursor.execute(query)
    food_lists=mycursor.fetchall()
    if len(food_lists)==0:
        messagebox.showinfo('Error','No food Found')
    else:
        def button_clicked(event):
            button = event.widget
            button_data = button_data_dict[button]
            global selected_food_id
            selected_food_id=int(button_data['text'])
            global selected_food_name
            selected_food_name=button_data['name']
            print("Button clicked - Name:", selected_food_name,"food id =" ,selected_food_id)
            order_food(selected_food_id)
        ...
        foodid=0
        i,j=0,0
        for i, food in enumerate(food_lists):
            for j, value in enumerate(food):
                a=40+40*i
                b=20+200*(j-1)
                ...
                if j==0:
                    ...
                    bttn_name = Button(dialog_frame, text=value,name=bttn_name, font=("Arial", 12),bd=0,bg=sc_color)
                    button_data_dict[bttn_name] = {'name': value, 'text': foodid}
                    bttn_name.place(x=b,y=a)
                    bttn_name.bind("<Button-1>", button_clicked)
                else:
                    label = Label(dialog_frame, text=value, font=("Arial", 12),bd=0,bg=sc_color)
        ...
        close_button = Button(dialog_frame, text="Order Now", command=order_food)
        close_button.place(x=300,y=300)
...
a4_label=Button(body_frame,text='Order Food',font=('Microsoft YaHei UI Light',10,'bold'),bg=sc_color,fg=pr_color,command=show_food)
