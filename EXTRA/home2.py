...
######################################
    ...
    new_ticket_id=len(ticket_list)+1
    global selected_ticket_id
...
######################################
    ...
    new_book_id=len(booking_list)+1
    book_status=0
...
def go_to_payment():
    ...
    connectDB()
    query='select seat_id from booking where book_status=0 and u_id=%s'
    mycursor.execute(query,(selected_user_id))
    ...
    if unpaid_books==None:
        ...
        for i,seat in enumerate(unpaid_books):
            a=100
            b=100+50*i
            Label(dialog_frame, text=seat, font=("Arial", 12),bd=0,bg=sc_color).place(x=a,y=b)
            '''
            query='UPDATE booking SET book_status=1 WHERE seat_id = %s;'
            mycursor.execute(query,(seat))
            query='UPDATE booking SET u_id=%s WHERE seat_id = %s;'
...
#show_tickets()[booked tickets]-> #-> cancel_ticket(t_id) -> ticket_clicked(tid)
